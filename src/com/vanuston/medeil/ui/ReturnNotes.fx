...
public class ReturnNotes {
    var log: Logger = Logger.getLogger(ReturnNotes.class,"Utilities");
    ...
    function buttonAction(): Void {
        if(txtRetNoteDetails.rawText.length()>0)
        {
           var expiryreturnController: com.vanuston.medeil.implementation.ExpiryReturn = RegistryFactory.getClientStub(RegistryConstants.ExpiryReturn) as com.vanuston.medeil.implementation.ExpiryReturn;
            ...
            expiryreturnBean.setExpiry_checkVal("updateReturnNotes");
            expiryreturnBean.setExpiry_dist_name(note_title);
            expiryreturnBean.setExpiry_batch_no(txtRetNoteDetails.rawText);
            expiryreturnBean.setExpiry_debit_note_no(note_no);
            //var expiryreturnController:ExpiryReturn = new  ExpiryReturnController();
            expiryreturnController.updateRecord(expiryreturnBean);
//            var sql = "UPDATE `{note_title.toLowerCase()}_note` SET details='{txtRetNoteDetails.rawText}' WHERE {note_title.toLowerCase()}_note_no='{note_no}'";
