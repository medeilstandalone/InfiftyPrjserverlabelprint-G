...
public class SalesReturnNotes {
    var log: Logger = Logger.getLogger(SalesReturnNotes.class,"Utilities");
    ...
    public-read def button3: javafx.scene.control.Button = javafx.scene.control.Button {
        ...
        onKeyPressed: button3OnKeyPressed
    ...
    function button3OnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
        saveCreditNotes();
        salesController.creditNotePrint(note_no,note_invoice);
        Hide();
         }
    function saveCreditNotes():Void {            
            var creditModel:CreditNoteModel  = new CreditNoteModel();
            creditModel.setDetails(txtRetNoteDetails.rawText);
            salesController.updateCreditDetails(creditModel.getDetails(),note_no);          
           
    }

    function button3OnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
            saveCreditNotes();
            salesController.creditNotePrint(note_no,note_invoice);
            Hide();
         }
